function create_res(names, file_sizes, buf_sizes, mode)
    ...
    for fi = 1:l_file
        for bi = 1:l_buf
            switch(mode)
                case 1
                    system(['./mmap.exe ' num2str(file_sizes(fi)) ' ' num2str(buf_sizes(bi)) ' >> ' filename]);
                case 2
                    system(['./shm.exe ' num2str(file_sizes(fi)) ' ' num2str(buf_sizes(bi)) ' >> ' filename]);
                case 3
                    system(['./recompile_msg.sh ' num2str(buf_sizes(bi))]);
                    system(['./msg.exe ' num2str(file_sizes(fi)) ' >> ' filename]);                    
            end
